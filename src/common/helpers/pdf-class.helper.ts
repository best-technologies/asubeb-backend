...
export interface ClassResultsRow {
  studentName: string;
  studentId: string;
  subjects: Record<string, { score: number; maxScore: number } | undefined>;
}
...
export interface ClassResultsPayload {
  ...
  sessionName: string;
  termName: string;
  subjects: string[]; // ordered unique subjects
  rows: ClassResultsRow[];
}

export function generateClassResultsPdf(payload: ClassResultsPayload): Promise<Buffer> {
  ...
  // Table header
  const tableLeft = 50;
  const tableRight = 801; // landscape width area
